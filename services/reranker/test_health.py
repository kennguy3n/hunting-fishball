"""gRPC health check tests for the reranker sidecar.

The health probe must succeed without instantiating the
cross-encoder model — these tests assert exactly that by asking
only the Health servicer and never the RerankerService.
"""
...
def test_health_check_reports_serving_for_named_service(health_channel):
    stub = health_pb2_grpc.HealthStub(health_channel)
    resp = stub.Check(
        health_pb2.HealthCheckRequest(service="reranker.v1.RerankerService")
    )
    assert resp.status == health_pb2.HealthCheckResponse.SERVING
