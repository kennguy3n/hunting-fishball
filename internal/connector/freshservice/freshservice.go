// Package freshservice implements the Freshservice SourceConnector +
// DeltaSyncer against the Freshservice REST API v2 at
// https://<domain>.freshservice.com/api/v2. Authentication uses HTTP
...
// `page` offset pagination (Freshservice caps at 30 pages × 100
// items, matching Freshdesk).
...
// WithBaseURL pins the Freshservice base URL — used by tests.
...
func (o *Connector) Connect(ctx context.Context, cfg connector.ConnectorConfig) (connector.Connection, error) {
	...
	base := o.baseURL
	if base == "" {
		base = "https://" + creds.Domain + ".freshservice.com"
	}
	...
	if resp.StatusCode == http.StatusTooManyRequests {
		return nil, fmt.Errorf("%w: freshservice: status=%d", connector.ErrRateLimited, resp.StatusCode)
	}
	if resp.StatusCode != http.StatusOK {
		return nil, fmt.Errorf("freshservice: agents/me status=%d", resp.StatusCode)
	}
...
func (it *docIterator) fetch(ctx context.Context) bool {
	...
	if status == http.StatusTooManyRequests {
		it.err = fmt.Errorf("%w: freshservice: list status=%d", connector.ErrRateLimited, status)

		return false
	}
	if status != http.StatusOK {
		it.err = fmt.Errorf("freshservice: list status=%d", status)

		return false
	}
	if decodeErr != nil {
		it.err = fmt.Errorf("freshservice: decode list: %w", decodeErr)

		return false
	}
	it.page = it.page[:0]
	it.idx = 0
	for _, t := range env.Tickets {
...
// ListDocuments enumerates Freshservice tickets.
func (o *Connector) ListDocuments(_ context.Context, c connector.Connection, ns connector.Namespace, _ connector.ListOpts) (connector.DocumentIterator, error) {
	...
	if !ok {
		return nil, errors.New("freshservice: bad connection type")
	}

	return &docIterator{o: o, conn: conn, ns: ns}, nil
}

// FetchDocument loads a Freshservice ticket by ID.
func (o *Connector) FetchDocument(ctx context.Context, c connector.Connection, ref connector.DocumentRef) (*connector.Document, error) {
	...
	defer func() { _ = resp.Body.Close() }()
	if resp.StatusCode == http.StatusTooManyRequests {
		return nil, fmt.Errorf("%w: freshservice: fetch status=%d", connector.ErrRateLimited, resp.StatusCode)
	}
	if resp.StatusCode != http.StatusOK {
		return nil, fmt.Errorf("freshservice: fetch %s status=%d", ref.ID, resp.StatusCode)
	}
	var env struct {
		Ticket ticketEntry `json:"ticket"`
	}
	if err := json.NewDecoder(resp.Body).Decode(&env); err != nil {
		return nil, fmt.Errorf("freshservice: decode ticket: %w", err)
	}
	t := env.Ticket
...
func (o *Connector) DeltaSync(ctx context.Context, c connector.Connection, ns connector.Namespace, cursor string) ([]connector.DocumentChange, string, error) {
	...
	if err != nil {
		return nil, "", fmt.Errorf("freshservice: bad cursor %q: %w", cursor, err)
	}
	const perPage = 100
	newest := cursorTime
	...
	for {
		...
		_ = resp.Body.Close()
		if status == http.StatusTooManyRequests {
			return nil, "", fmt.Errorf("%w: freshservice: status=%d", connector.ErrRateLimited, status)
		}
		if status != http.StatusOK {
			return nil, cursor, fmt.Errorf("freshservice: delta status=%d", status)
		}
		if decodeErr != nil {
			return nil, "", fmt.Errorf("freshservice: decode delta: %w", decodeErr)
		}
		for _, t := range env.Tickets {
...
func (o *Connector) do(ctx context.Context, conn *connection, method, path string, body io.Reader) (*http.Response, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("freshservice: build request: %w", err)
	}
	req.Header.Set("Authorization", conn.authHdr)
	req.Header.Set("Accept", "application/json")
	resp, err := o.httpClient.Do(req)
	if err != nil {
		return nil, fmt.Errorf("freshservice: %s %s: %w", method, target, err)
	}
